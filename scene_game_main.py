...
with st.sidebar:
    ...
    # with tab2:
    # # 提供音频文件
    #     audio_file = open(r'./peace.mp3', 'rb')
    #     st.audio(audio_file, format='audio/mp3', start_time=0)
    # with tab3:
    #     audio_file3 = open(r'./tension.mp3', 'rb')
...
def display_sidebar():
    st.sidebar.markdown("<style>.SR, .W, .S, .A, .H{font-size:24px; font-weight: bold;}</style>",unsafe_allow_html=True)
