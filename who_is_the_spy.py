...
def generate0(prompt):

    response = client.chat.completions.create(
        model='glm-4',
        messages = [{"role":"system", "content":prompt},
                    {"role":"system", "content":"/describe "+"请根据描述历史记录描述你的关键词，"
                                                             "需要注意不能与已有的描述重复，下面是描述历史记录：\n"+"\n".join(state.description)}],
        temperature=0.7,
        max_tokens=2048,
        top_p=0.75
        )
...
def generate1(prompt):
    response = client.chat.completions.create(
        model='glm-4',
        messages = [
        {"role": "system", "content": state.prompt.format(prompt)},
    ...
    if start:
        ...
        for player in state.players:
            ...
            if player["dignity"] == "spy":
                text = generate0(prompt=state.prompt.format(state.spy_word))
                # text = response([
                ...
                text = generate0(prompt=state.prompt.format(state.civilian_word))
                # text = response([
    ...
    with col2:
        if st.button("开始投票"):
            for player in state.players:
                ...
                text = generate1(prompt=state.prompt.format(state.spy_word))
                # text = response([
                #     {"role":"system", "content":state.prompt.format(state.spy_word)},
                #     {"role":"system", "content":"/vote "+"请根据描述历史记录选择要投出的玩家，下面是描述历史记录：\n"+"\n".join(state.description)+"\n"+"当前场上存活玩家id为：\n"+",".join([a["id"] for a in state.players])}
                #     ])
